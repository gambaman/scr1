...
ENDIANNESS | endianness treatment | **BE** **AEBO** **LE**

If ENDIANNESS=LE samples are supposed to be written in the native little-endian byte order. If ENDIANNESS=BE they are supposed to be written in big-endian and byte order conversions are carried out in software. If ENDIANNESS=AEBO the samples are supposed to be written in big-endian and byte order conversions are carried using the ABEO extension.

By default, PLATFORM=nexys4ddr_scr1, OPT=2 and ENDIANNESS=LE argument values are used.
